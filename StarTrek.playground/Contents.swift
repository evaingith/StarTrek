...
let 👩 = humanOfficers.filter{femaleOfficers.contains($0)}.count
let percent = Float(👩)/Float(humanOfficers.count)


// Q2: How many first officers are human?
let firstHumanOfficers = firstOfficers.keys.filter{return humanOfficers.contains($0)}
print(firstOfficers.count)

// Q3: Get a list of females sorted by name
print(femaleOfficers.sorted(by: <))
...
let sortedCapitains = allCaptains.sorted{$0.name < $1.name}
print(sortedCapitains)
...
let firstVulcanOfficers = first.filter{vulcanOfficers.contains($0.name)}
print(firstVulcanOfficers)
